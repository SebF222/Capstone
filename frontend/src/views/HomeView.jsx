...
const HomeView = () => {
  ...
  const loadPastGames = async () => {
    setIsLoading(true);
    try {
      const response = await fetch(`${API_URL}/sports/${selectedSport}/games/date?date=${selectedDate}`);
      const data = await response.json();

      if (response.ok && data.response) {
        setPastGames(data.response.slice(0, 10));
      } else {
        setPastGames([]);
      }
    } catch (error) {
  ...
  const filteredLiveGames = liveGames.filter((game) => {
    if (!searchQuery) return true; // no filter if empty
    ...
    return homeName.includes(q) || awayName.includes(q) || homeId.includes(q) || awayId.includes(q);
  });
      ...
      <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '2rem' }}>
        ...
        {user && favorites.length > 0 && (
          <div style={{ backgroundColor: 'white', borderRadius: '0.5rem', padding: '1.5rem', marginBottom: '2rem', boxShadow: '0 1px 3px 0 rgba(0, 0, 0, 0.1)' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '1rem' }}>
              {favorites.map((fav) => (
                <div key={fav.id} style={{ padding: '1rem', border: '1px solid #e5e7eb', borderRadius: '0.375rem', display: 'flex', alignItems: 'center', gap: '0.75rem', cursor: 'pointer', transition: 'all 0.2s' }}
                  ...
                  <img src={fav.team_logo} alt={fav.team_name} style={{ width: '40px', height: '40px', objectFit: 'contain' }} />
                  <div>
            ...
            <NavLink to="/favorites" style={{ display: 'inline-block', marginTop: '1rem', color: '#2563eb', fontSize: '0.875rem', textDecoration: 'none', fontWeight: '500' }}>
              Manage Favorites →
            </NavLink>
          </div>
        )}
          ...
          {isLoading ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              {filteredLiveGames.map((game, index) => (
                <div key={index} style={{ padding: '1rem', border: '1px solid #e5e7eb', borderRadius: '0.375rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', flex: 1 }}>
                    <div style={{ textAlign: 'center', minWidth: '100px' }}>
                      <div style={{ fontWeight: '600' }}>{game.teams?.home?.name || 'Home Team'}</div>
                      <div style={{ fontSize: '0.75rem', color: '#6b7280' }}>HOME</div>
                    </div>
                    <div style={{ fontSize: '1.5rem', fontWeight: 'bold', color: '#2563eb' }}>VS</div>
                    <div style={{ textAlign: 'center', minWidth: '100px' }}>
                      <div style={{ fontWeight: '600' }}>{game.teams?.away?.name || 'Away Team'}</div>
                      <div style={{ fontSize: '0.75rem', color: '#6b7280' }}>AWAY</div>
                    </div>
                  </div>
                  <div style={{ textAlign: 'right' }}>
                    <div style={{ display: 'inline-block', padding: '0.25rem 0.75rem', backgroundColor: '#fee2e2', color: '#991b1b', borderRadius: '9999px', fontSize: '0.75rem', fontWeight: '600' }}>LIVE</div>
                    <div style={{ fontSize: '0.75rem', color: '#6b7280', marginTop: '0.25rem' }}>{game.status?.long || 'In Progress'}</div>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div style={{ textAlign: 'center', padding: '3rem' }}>
              ...
              <p style={{ color: '#6b7280', fontSize: '1.125rem' }}>No live games found</p>
            </div>
          )}
        </div>


        <div style={{ backgroundColor: 'white', borderRadius: '0.5rem', padding: '1.5rem', marginTop: '2rem', boxShadow: '0 1px 3px 0 rgba(0, 0, 0, 0.1)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
            <h3 style={{ fontSize: '1.25rem', fontWeight: 'bold', color: '#111827' }}>
              📅 Past Games - {sportTabs.find(s => s.id === selectedSport)?.name}
            </h3>
            <input
              type="date"
              value={selectedDate}
          ...
          {isLoading ? (
            <div style={{ textAlign: 'center', padding: '2rem' }}>
              <div style={{ fontSize: '2rem', marginBottom: '1rem' }}>⏳</div>
              <p style={{ color: '#6b7280' }}>Loading past games...</p>
            </div>
          ) : filteredPastGames.length > 0 ? (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              {filteredPastGames.map((game, index) => (
                <div key={index} style={{ padding: '1rem', border: '1px solid #e5e7eb', borderRadius: '0.375rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', flex: 1 }}>
                    <div style={{ textAlign: 'center', minWidth: '100px' }}>
                      <div style={{ fontWeight: '600' }}>{game.teams?.home?.name || 'Home Team'}</div>
            ...
            <div style={{ textAlign: 'center', padding: '3rem' }}>
              <div style={{ fontSize: '3rem', marginBottom: '1rem' }}>📅</div>
              <p style={{ color: '#6b7280', fontSize: '1.125rem' }}>No games found for this date</p>
            </div>
          )}
        </div>
        ...
        {!user && (
          <div style={{ marginTop: '2rem', textAlign: 'center', padding: '2rem', backgroundColor: 'white', borderRadius: '0.5rem', boxShadow: '0 1px 3px 0 rgba(0, 0, 0, 0.1)' }}>
            <h3 style={{ fontSize: '1.5rem', fontWeight: 'bold', marginBottom: '1rem' }}>Want to track your favorite teams?</h3>
            <p style={{ color: '#6b7280', marginBottom: '1.5rem' }}>Create an account to save your favorite teams and get personalized updates</p>
            <div style={{ display: 'flex', gap: '1rem', justifyContent: 'center' }}>
              <NavLink to="/signup" style={{ display: 'inline-block', padding: '0.75rem 2rem', backgroundColor: '#2563eb', color: 'white', borderRadius: '0.375rem', textDecoration: 'none', fontWeight: '500' }}>Sign Up</NavLink>
              <NavLink to="/login" style={{ display: 'inline-block', padding: '0.75rem 2rem', backgroundColor: 'white', color: '#2563eb', border: '2px solid #2563eb', borderRadius: '0.375rem', textDecoration: 'none', fontWeight: '500' }}>Log In</NavLink>
            </div>
          </div>
        )}
      </div>
